...
ServFlow is an agent builder: you define AI agents and the workflows they run, and a Go engine serves them. Alongside the visual builder, every object can be defined as a JSON or YAML configuration file and deployed with the `servflowai` CLI — which is what you want for version control, code review, and CI/CD.
...
- ServFlow installed — see [Installation](/installation)
...
```bash
servflowai resource config example --config config.toml --output yaml
```
...
```bash
servflowai resource config create \
...
```bash
servflowai resource config create -f configs/greeting.yaml --config config.toml
...
```bash
# every action type
servflowai resource action list --config config.toml

# the fields one action accepts
servflowai resource action describe static --config config.toml

# every template function
servflowai resource config template-functions --config config.toml
```

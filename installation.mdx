---
title: "Installation"
description: "Install ServFlow, start the server, and create your first account"
icon: "download"
---

ServFlow is an agent builder: you define AI agents and the workflows they run, and a Go engine serves them. This guide installs the `servflowai` binary, starts an instance, and walks through the first-run account setup.
...
# Or Docker
docker run -p 8080:8080 servflow/servflowai
```
...
Download pre-built binaries from the [Releases](https://github.com/Servflow/servflowai/releases) page. Available for **Linux** (x86_64, arm64) and **macOS** (x86_64, arm64).
...
```bash
servflowai resource config list --config config.toml
```

...
Runs another workflow and returns its result. The target must be a workflow with a trigger entry — a workflow is reached only by invocation, never by an HTTP request.

### Workflow ID

The `config_id` of the workflow to run. This is fixed at author time, not templated, so the target is known up front.
...
Values to pass to the target workflow, as a map of input name to template. Each template is resolved against the **calling** workflow's request.
...
Inside the target workflow, read them with `{{ input "name" }}`.

### Output

`{{ .step_id }}` holds whatever the called workflow returned, parsed as JSON when it is JSON.

<Note>
A workflow cannot call itself, directly or through a chain. The cycle is caught at runtime.
</Note>

...
The `provider` noun of the `resource` command manages stored providers. `list` and `get` never return a provider's key.

| Command | What it does |
|---|---|
| `servflowai resource provider list` | Lists providers. |
| `servflowai resource provider get <id>` | Gets one provider. |
| `servflowai resource provider create -f <file>` | Creates a provider. |
| `servflowai resource provider update <id> -f <file>` | Renames a provider and changes the fields you give. |
| `servflowai resource provider delete <id>` | Deletes a provider. |
| `servflowai resource provider types` | Lists the provider types. |
| `servflowai resource provider describe [type...]` | Describes a type's fields and suggested models. Pass `--all` for every type. |

The body needs a name, a type, and a config:
...
`create` checks the config by building the connection it describes, so a missing field is reported at once. `--dry-run` validates without writing.
...
<Card title="Providers reference" icon="cloud" href="/references/providers">

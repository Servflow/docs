...
Leave a branch unset to end the workflow when it is taken.
...
When validation functions like `notempty`, `email`, and `bcrypt` include a title
parameter, validation errors are collected automatically. Any later step can read
them with the `.errors` variable.
...
`.errors` holds one message per failed check, using the titles given to the
validation functions:

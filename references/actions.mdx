...
\n## Shell action environments\n\nServFlow shell actions now support setting environment variables unique to each step and command. Every shell step runs in its own folder, and custom environment values can be provided to scripts. For technical plan details, see the implementation notes in the [servflowai repository](https://git.servflow.io/servflow/servflowai/src/branch/feat/shell-env/docs/shell-env-plan.md).\n

...
The body names the id, the type, and the config as a JSON object. Each field in the config is an object holding either a literal, as `{"value": ...}`, or the name of a stored secret, as `{"secret": "<secret name>"}`:
...
`create` and `update` take `--dry-run`, which validates the body and prints it without writing.

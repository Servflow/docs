...
The [Entry handlers reference](/references/entry-handlers) lists every variable of each handler.
...
| `secret(name)` | A stored secret, or the environment variable of the same name. See [Secrets](/references/secrets). | `{{ secret "weather-api-key" }}` |

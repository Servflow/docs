...
| `expression` | Allow the call when | string | One of `expression` or `structure` | A template that has to render `true`, such as `{{ eq (body "plan") "pro" }}`. See [Dynamic content](/references/dynamic-content). |
